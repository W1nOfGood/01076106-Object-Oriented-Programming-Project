...
class Subject:
    ...
    def assign_teacher(self, teacher):
        if teacher.__class__ is not Teacher:
            return "Error"
...
def _is_student(obj):
    return obj.__class__ is Student


def _is_subject(obj):
    return obj.__class__ is Subject


def _is_teacher(obj):
    return obj.__class__ is Teacher

# TODO 1 : function สำหรับค้นหา instance ของวิชาใน subject_list
# TODO 1 : Function for finding an instance of a subject in subject_list.
def search_subject_by_id(subject_id):
    for subject in subject_list:
        if _is_subject(subject) and subject.subject_id == subject_id:
            return subject
    return None


# TODO 2 : function สำหรับค้นหา instance ของนักศึกษาใน student_list
# TODO 2 : Function for finding an instance of a student in student_list.
def search_student_by_id(student_id):
    for student in student_list:
        if _is_student(student) and student.student_id == student_id:
            return student
    return None


# TODO 3 : function สำหรับสร้างการลงทะเบียน โดยรับ instance ของ student และ subject
# TODO 3 : Function for creating an enrollment, taking instances of a student and a subject as input.
def enroll_to_subject(student, subject):
    if not _is_student(student) or not _is_subject(subject):
        return "Error"
...
# TODO 4 : Function for removing an enrollment, taking instances of a student and a subject as input.
...
# TODO 5 : Function for finding an enrollment, taking instances of a student and a subject as input.
def search_enrollment_subject_student(subject, student):
    ...
    for enrollment in enrollment_list:
        if (
            ...
            and enrollment.subject.subject_id == subject.subject_id
        ):
            return enrollment
    return None
...
# TODO 6 : Function for finding enrollments in a specific subject, taking an instance of the subject as input.
def search_student_enroll_in_subject(subject):
    if not _is_subject(subject):
        return "Error"
    return [enroll for enroll in enrollment_list if enroll.subject.subject_id == subject.subject_id]


# TODO 7 : function สำหรับค้นหาการลงทะเบียนของนักศึกษาว่ามีวิชาอะไรบ้าง โดยรับ instance ของ student
# TODO 7 : Function for finding the subjects a student has enrolled in, taking an instance of the student as input.
def search_subject_that_student_enrolled(student):
    if not _is_student(student):
        return "Error"
    filtered = [enroll for enroll in enrollment_list if enroll.student.student_id == student.student_id]
    return filtered if filtered else "Not Found"


# TODO 8 : function สำหรับใส่เกรดลงในการลงทะเบียน โดยรับ instance ของ student และ subject
# TODO 8 : Function for assigning grades in an enrollment, taking instances of a student, a subject, and the grade as input
def assign_grade(student, subject, grade):
    if not _is_student(student) or not _is_subject(subject) or grade.__class__ is not str:
        return "Error"
...
# TODO 9 : Function for returning an instance of the teacher teaching a subject.
def get_teacher_teach(subject_search):
    if not _is_subject(subject_search):
        return "Error"
    subject = search_subject_by_id(subject_search.subject_id)
    if subject is None or subject.teacher is None:
        return "Not Found"
    return subject.teacher


# TODO 10 : function สำหรับค้นหาจำนวนของนักศึกษาที่ลงทะเบียนในรายวิชา โดยรับ instance ของ subject
# TODO 10 : Function for finding the number of students enrolled in a subject, taking an instance of the subject as input.
def get_no_of_student_enrolled(subject):
    if not _is_subject(subject):
        return "Error"
    enrolled = search_student_enroll_in_subject(subject)
    if enrolled == "Error":
        return "Error"
    if len(enrolled) == 0:
        return "Not Found"
...
# TODO 11 : Function for finding enrollment and academic performance data for a student,
# taking an instance of the student as input.
# Returns a dictionary in the format: { 'subject_id': ['subject_name', 'grade'] }.
def get_student_record(student):
    ...
    if enrollments == "Error":
        return "Error"
    if enrollments == "Not Found":
        return {}
    record = {}
    for enrollment in enrollments:
        record[enrollment.subject.subject_id] = [
...
# Function to convert grades to numeric values: Converts grades into numerical equivalents.
...
# TODO 12 : Function for calculating a student's GPA, taking an instance of the student as input.
def get_student_GPS(student):
    if not _is_student(student):
        return "Error"
    enrollments = search_subject_that_student_enrolled(student)
    if enrollments in ("Error", "Not Found"):
        return 0
    total_credit = 0
    total_grade_point = 0
    for enrollment in enrollments:
        ...
        total_credit += credit
        total_grade_point += grade_to_count(enrollment.grade) * credit
    if total_credit == 0:
        return 0
    return total_grade_point / total_credit

# ค้นหานักศึกษาลงทะเบียน โดยรับเป็น รหัสวิชา และคืนค่าเป็น dictionary {รหัส นศ. : ชื่อ นศ.}
# Function to find enrolled students by subject ID: Takes a subject ID and
# returns a dictionary in the format {student_id: student_name}.
...
# Function to find subjects enrolled by a student ID: Takes a student ID and
# returns a dictionary in the format {subject_id: subject_name}.
def list_subject_enrolled_by_student(student_id):
    ...
    if filter_subject_list in ("Error", "Not Found"):
        return {}
    subject_dict = {}
